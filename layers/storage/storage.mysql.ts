 ...
 */
export const mysqlStorage = <T extends { id: string }>(
  db: Db,
  ...
  return {
    ...
    getById: (id) =>
      Effect.gen(function* () {
        ...
        if (rows.length === 0) return yield* Effect.fail(new EntityNotFound(id))
        return rows[0]
      }),

    create: (item) =>
      tryDb(() => db.insert(table).values(item as any)).pipe(
        Effect.as(item),
      ),
    ...
    update: (id, patch) =>
      Effect.gen(function* () {
        ...
        yield* tryDb(() =>
          db.update(table).set(patch as any).where(eq(table.id, id)),
        )
